...
| [추천 시스템](recommender-systems.md) | 핵심 |
| [멀티모달 모델: 텍스트·이미지·오디오](multimodal-models.md) | 응용 |
| [모델 효율화: 양자화·프루닝·증류](model-efficiency.md) | 응용 |
| [AI 에이전트와 도구 오케스트레이션](ai-agents.md) | 응용 |

학습 경로: 확률·통계 → 모델 선택 → 앙상블·차원 축소·SVM / 최적화 → 역전파 → 학습 안정성 → 강화학습 / 차원 축소 → 오토인코더 → 생성 모델·추천 시스템 / 신경망 → 그래프 신경망 / Transformer → 멀티모달 → 사전·후학습 → 검색·메모리 → LLM 평가.
