...
try{
 ...
 assert.ok(await page.locator('.doc-card').count()<10);
 await page.locator('#search').fill('');
 for(const level of ['기초','핵심','참고','가이드']){
  await page.locator(`[data-filter="${level}"]`).click();
  assert.equal(await page.locator('.doc-card').count(),os.filter(d=>d.level===level).length);
 }
 await page.locator('[data-filter="전체"]').click();
 await page.screenshot({path:'.preview/os-desktop.png',fullPage:true});
 for(const doc of os){
  await page.goto(base+'#/os/'+doc.slug);
 ...
 for(const doc of os){
  ...
  if(source){
   assert.equal(await page.locator('.os-original').innerText(),source.quote);
   assert.ok(await page.locator('.article a').evaluateAll((as,url)=>as.some(a=>a.href===url),source.url));
   assert.match(await page.locator('.source-link').getAttribute('href'),/sources\/os.json$/);
  }
 }
 await page.reload();
 ...
 await page.goto(base+'#/os/virtual-memory');
 await page.waitForSelector('.concept-diagram');
 await page.screenshot({path:'.preview/os-mobile.png',fullPage:true});
