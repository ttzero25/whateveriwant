...
try{
  ...
  for(const doc of security){
    ...
    if(topic==='vulnerabilities')assert.equal(await page.locator('.concept-diagram').count(),['sql-injection','access-control','xss','csrf','ssrf','command-injection','nosql-ldap-injection','mass-assignment'].includes(doc.slug)?1:0);
