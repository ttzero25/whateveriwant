...
const topics={network:{name:'Network',icon:'⇄'},os:{name:'Operating Systems',icon:'▤'},cs:{name:'Computer Science',icon:'⌘'},ai:{name:'Artificial Intelligence',icon:'✳'},security:{name:'Security',icon:'◇'},'ai-for-security':{name:'AI for Security',icon:'⛨'}};
const topicName=topic=>topics[topic]?.name||topic;
...
function article(topic,slug){
 ...
 const figure=topic==='ai'?diagram(slug,lang):['cs','os','network'].includes(topic)?diagram(topic+'-'+slug,lang):'';
...
function route(){localizeShell();closeMenu();const hash=location.hash||'#/',[,topic,slug]=hash.split('/');document.querySelector('.home-link').classList.toggle('active',hash==='#/');document.querySelector('.ai-link').classList.toggle('active',activeTopic()==='ai');for(const topicKey of ['security','ai-for-security','cs','os','network'])document.querySelector('.'+topicKey+'-link').classList.toggle('active',topic===topicKey);document.querySelector('.research-link').classList.toggle('active',topic==='research');if(topic==='research'&&!slug)renderResearch(main,lang);else if(slug)article(topic,slug);else library();window.scrollTo(0,0);}
