...
export function relatedConcepts(item,documents){
 const defaults=item.tags.includes('security')?['ai-for-security/securing-ai-systems','security/principles']:item.tags.includes('evaluation')?['ai/evaluation','ai-for-security/detection-evaluation']:['ai-for-security/securing-ai-systems','ai/evaluation'];
...
export async function renderHomeUpdates(container,lang,documents){
 ...
 try{
  ...
  if(!active())return;
  const items=data.items.filter(item=>item.tags.some(tag=>['security','safety','evaluation'].includes(tag))).sort((a,b)=>b.date.localeCompare(a.date)||a.title.localeCompare(b.title)).slice(0,3);
  container.innerHTML=`<div class="updates-heading"><div><div class="eyebrow">LATEST UPDATES</div><h2>${t('지금 읽어볼 AI 보안·연구 이슈','Latest AI security & research')}</h2><p>${t('새로운 이슈를 읽고, 이해에 필요한 개념으로 이어가세요.','Read the latest updates, then explore the concepts behind them.')}</p></div><a class="updates-all" href="#/research">${t('동향 전체 보기','All updates')} →</a></div><div class="updates-grid">${items.length?items.map(item=>{
   const source=data.sources.find(s=>s.id===item.source);
   const stale=!source?.last_success||Date.now()-Date.parse(source.last_success)>48*3600000||source.status==='error';
  ...
  }).join(''):`<p class="updates-empty">${t('수집된 관련 이슈가 아직 없어요. 동향 페이지에서 다른 주제를 살펴보세요.','No related updates collected yet. Explore other topics on the updates page.')}</p>`}</div><p class="updates-note">${t('수집된 보안·안전성·평가 글 중 최신 3건 · 날짜는 출처의 게시·공고 기준 · 관련 개념은 제목·주제 기반 자동 연결','Latest three collected security, safety and evaluation entries · Source publication or announcement dates · Concept links suggested from titles and topics')}</p>`;
 }catch{
  if(!active())return;
