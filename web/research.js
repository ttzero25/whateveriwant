import {setupAIResearchFeed} from './robotics-archive.js';
import {renderTLDR,setupConferences} from './research-tldr.js';
const escape=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const aiLabels={security:['AI 보안','AI security'],safety:['안전성·정렬','Safety & alignment'],evaluation:['평가·신뢰성','Evaluation'],research:['기타 동향','Other updates']};
export function createResearchWatch(config={}){
const labels=config.labels||aiLabels,route=config.route||'#/research';
const title=config.title||'AI Research Watch';
let loadedAt=0;
let request,source='all',topic='all',days='30',query='';
function loadResearch(){
 if(Date.now()-loadedAt>300000){request=null;loadedAt=Date.now();}
 request??=fetch(config.file||'./research.json',{cache:'no-cache'}).then(r=>{if(!r.ok)throw Error('Load failed');return r.json();}).catch(e=>{request=null;throw e;});
 return request;
}
async function renderResearch(main,lang){
 const t=(ko,en)=>lang==='en'?en:ko;
 document.title=title+' — whateveriwant';
 ...
 try{
  ...
  if(location.hash!==route||document.documentElement.lang!==lang)return;
  ...
  function update(reset=true){
   ...
   const matches=data.items.filter(item=>(source==='all'||item.source===source)&&(topic==='all'||item.tags.includes(topic))&&item.date>=cutoff&&[item.title,item.tldr?.ko,item.tldr?.en].filter(Boolean).join(' ').toLowerCase().includes(query.toLowerCase().trim())).filter((item,index,items)=>items.findIndex(other=>other.url===item.url)===index);
   main.querySelector('#research-count').textContent=t(`${matches.length}개 항목`,`${matches.length} entries`);
   main.querySelector('#research-results').innerHTML=matches.length?matches.slice(0,visible).map(item=>`<article class="research-item"><div class="research-item-meta"><span>${escape(data.sources.find(s=>s.id===item.source)?.name||item.source)}</span><time datetime="${item.date}">${item.date}</time><small>${item.date_kind==='announced'?(item.announcement?.startsWith('replace')?t('수정 공고','Revision announced'):item.announcement==='new'?t('신규 공고','New submission'):t('공고일','Announced')):t('게시일','Published')}</small>${item.kind==='preprint'?'<span class="research-badge">PREPRINT</span>':`<span class="research-badge">${item.kind==='community'?t('커뮤니티','Community'):item.channel==='news'?t('뉴스·발표','News'):t('연구','Research')}</span>`}</div><h3><a href="${escape(item.url)}" lang="en" target="_blank" rel="noopener noreferrer">${escape(item.title)} <span aria-hidden="true">↗</span></a></h3>${renderTLDR(item,lang)}<div class="research-tags">${item.tags.map(tag=>`<span>${escape(labels[tag]?.[lang==='en'?1:0]||tag)}</span>`).join('')}</div></article>`).join(''):`<div class="empty">${t('이 조건에 맞는 글이 없어요. 기간을 넓히거나 다른 출처·주제를 선택해보세요.','No matching entries. Try a wider period or another source or topic.')}</div>`;
...
return {loadResearch,renderResearch};
}
export const {loadResearch,renderResearch}=createResearchWatch({setupFeed:setupAIResearchFeed,feedId:'ai-feed'});
