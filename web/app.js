...
function article(topic,slug){
 ...
 const figure=topic==='ai'?diagram(slug,lang):['cs','os','network','vulnerabilities','crypto','algorithms','security','ai-for-security'].includes(topic)?diagram(topic+'-'+slug,lang):'';
...
function route(){localizeShell();closeMenu();const hash=location.hash||'#/',[,topic,slug]=hash.split('/');document.querySelector('.home-link').classList.toggle('active',hash==='#/');document.querySelector('.ai-link').classList.toggle('active',activeTopic()==='ai');for(const topicKey of ['security','ai-for-security','cs','os','network','vulnerabilities','crypto','algorithms'])document.querySelector('.'+topicKey+'-link').classList.toggle('active',topic===topicKey);document.querySelector('.research-link').classList.toggle('active',topic==='research');document.querySelector('.robotics-link').classList.toggle('active',topic==='robotics-security');document.querySelector('.security-news-link').classList.toggle('active',topic==='security-news');if(topic==='security-news'&&!slug)renderSecurityNews(main,lang);else if(topic==='robotics-security'&&!slug)renderRobotics(main,lang);else if(topic==='research'&&!slug)renderResearch(main,lang);else if(slug)article(topic,slug);else library();window.scrollTo(0,0);}
