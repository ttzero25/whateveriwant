import {renderSecurityNews,renderSecurityBanner} from './security-news.js';
import {renderDailyUpdates} from './daily-updates.js';
...
function localizeShell(){
 ...
 document.querySelector('.security-news-title').textContent=t('보안 이슈 Watch','Security News Watch');
 updateAIExpansion();
 ...
 document.querySelector('.crypto-link').innerHTML=`<span>◆</span> ${topicName('crypto')} <span class="nav-count">${topicCount('crypto')}</span>`;
 document.querySelector('.algorithms-link').innerHTML=`<span>∑</span> ${topicName('algorithms')} <span class="nav-count">${topicCount('algorithms')}</span>`;
 document.querySelector('.security-for-ai-link').innerHTML=`<span>⬢</span> ${topicName('security-for-ai')} <span class="nav-count">${topicCount('security-for-ai')}</span>`;
...
function library(){
 ...
 if(!activeTopic()){renderDailyUpdates(document.querySelector('#home-today'),lang);renderHomeUpdates(document.querySelector('#home-updates'),lang,documents);renderSecurityBanner(document.querySelector('#home-security-news'),lang);}
}
function article(topic,slug){
 ...
 const meta=lang==='en'?(doc.type==='korean'?'AI-authored explanation':doc.type==='navigation'?'Site navigation · authored for whateveriwant':doc.type==='excerpt'?'Official reference · short original excerpt':'Google · CC BY 4.0 · original excerpts'):'AI 작성 해설';
...
function route(){localizeShell();closeMenu();const hash=location.hash||'#/',[,topic,slug]=hash.split('/');document.querySelector('.home-link').classList.toggle('active',hash==='#/');document.querySelector('.ai-link').classList.toggle('active',activeTopic()==='ai');for(const topicKey of ['security','ai-for-security','cs','os','network','vulnerabilities','crypto','algorithms','security-for-ai'])document.querySelector('.'+topicKey+'-link').classList.toggle('active',topic===topicKey);document.querySelector('.research-link').classList.toggle('active',topic==='research');document.querySelector('.robotics-link').classList.toggle('active',topic==='robotics-security');document.querySelector('.security-news-link').classList.toggle('active',topic==='security-news');if(topic==='security-news'&&!slug)renderSecurityNews(main,lang);else if(topic==='robotics-security'&&!slug)renderRobotics(main,lang);else if(topic==='research'&&!slug)renderResearch(main,lang);else if(slug)article(topic,slug);else library();window.scrollTo(0,0);}
